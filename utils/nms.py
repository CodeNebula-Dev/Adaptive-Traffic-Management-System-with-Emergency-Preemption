...
def class_aware_nms(predictions, conf_threshold=0.25, iou_threshold=0.45,
    ...
    # Limit candidate detections before containment to prevent O(N^2) bottlenecks
    if len(keep) > max_detections:
        keep = keep[:max_detections]

    # Spatial Containment De-duplication (Vectorized, O(1) tensor kernel):
    # If a smaller box is largely (>70%) nested inside a larger higher-scoring box,
    # suppress the internal sub-box.
    if len(keep) > 1:
        s_boxes = boxes[keep]
        # Pairwise intersection: (K, K)
        ix1 = torch.maximum(s_boxes[:, None, 0], s_boxes[None, :, 0])
        iy1 = torch.maximum(s_boxes[:, None, 1], s_boxes[None, :, 1])
        ix2 = torch.minimum(s_boxes[:, None, 2], s_boxes[None, :, 2])
        iy2 = torch.minimum(s_boxes[:, None, 3], s_boxes[None, :, 3])

        inter = torch.clamp(ix2 - ix1, min=0.0) * torch.clamp(iy2 - iy1, min=0.0)
        s_areas = (s_boxes[:, 2] - s_boxes[:, 0]) * (s_boxes[:, 3] - s_boxes[:, 1])

        # Overlap fraction of box j inside higher-confidence box i (where i < j):
        containment = inter / (s_areas[None, :] + 1e-6)
        upper_tri = torch.triu(containment > 0.70, diagonal=1)
        suppressed = upper_tri.any(dim=0)
